        ...
        const UMLRTCapsuleClass * slotClass, const UMLRTCommsPort * * borderPorts, bool bind, bool import )
{
    ...
    for (size_t slot_i = 0; (slot_i < slotClass->numPortRolesBorder) && compatible; ++slot_i)
    {
        ...
        while (compatible && !match)
        {
            ...
            // If the border port named matched the slot port, match is true or compatible is false.
    ...
    if (compatible && bind)
    {
                    ...
                    if (import && bind)
                    {
                        ...
                        if (!existingBorderPort->unbound)
                        {
                            FATAL("import binding of capsule %s into slot %s assigning slot port %s was not 'unbound'",
                                    capsule->getName(), slot->name, existingBorderPort->role()->name );
                        }
        ...
        for (size_t i = 0; i < requestedClass->numPortRolesBorder; ++i)
        {
            ...
            {
                // The newBorderPort remained NULL during binding. For import, the border port should already be the unbound proxy port.
...
/*static*/ const UMLRTCommsPort * UMLRTFrameService::bindPortsCreateProxyPort ( UMLRTSlot * slot, UMLRTCapsule * capsule, const UMLRTCommsPort * * borderPorts, const UMLRTCommsPort * slotPort, const UMLRTCapsuleClass * requestedClass, int border_i, bool bind, bool import )
{
    ...
    if (import && bind)
    {
        const UMLRTCommsPort * existingBorderPort = capsule->getBorderPorts()[border_i];
        if (!existingBorderPort->unbound)
        {
        ...
        capsule->getBorderPorts()[border_i] = newBorderPort;
    }
...
/*static*/ void UMLRTFrameService::sendBoundUnbound( const UMLRTCommsPort * internalPorts, int portId, int farEndIndex, bool isBind )
{
    if (internalPorts != NULL)
    {
        const UMLRTCommsPort * port = &internalPorts[portId];
        if (!port->unbound)
        {
...
/*static*/ void UMLRTFrameService::sendBoundUnbound( const UMLRTCommsPort * * borderPorts, int portId, int farEndIndex, bool isBind )
{
    if (borderPorts != NULL)
    {
        const UMLRTCommsPort * port = borderPorts[portId];
        if (!port->unbound)
        {
            if (farEndIndex < (int)port->numFarEnd)
            {
