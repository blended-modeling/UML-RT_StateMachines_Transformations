 ...
 */
public class CSMDRegionSyncBucket extends UMLRTMasterSlaveSyncBucket<Region> {

	public CSMDRegionSyncBucket(Region model, EditPart master) {
		super(model, master);

		add(new NodeSizeSyncFeature<>(this));
		add(new NodePositionSyncFeature<>(this));
		add(new CSMDRegionVerticesSyncFeature(this));
	}
