...
/**
 * Specialization of the Operation as message in message sets, but specific to message sets with kind=OUT
 */
public class OperationAsMessageOutEditHelperAdvice extends AbstractEditHelperAdvice {
	...
	public boolean approveRequest(IEditCommandRequest request) {
		...
		if (request instanceof CreateElementRequest) {
			...
			if ((container instanceof Operation) && ElementTypeUtils.isTypeCompatible(((CreateElementRequest) request).getElementType(), UMLElementTypes.PARAMETER)) {
				approve = true;
			...
			if (messageSetType instanceof ISpecializationType) { // check at the same time UMLRT element types are correctly loaded
				...
				if (((ISpecializationType) messageSetType).getMatcher().matches(container)) {
					...
					// Approve the request only if the Message matches the OUT type
					approve = true;
				}
			}
	...
	protected ICommand getAfterConfigureCommand(ConfigureRequest request) {
		...
		final String name = NamedElementUtil.getDefaultNameWithIncrementFromBase("OutProtocolMessage", operationAsMessageIn.eContainer().eContents());
