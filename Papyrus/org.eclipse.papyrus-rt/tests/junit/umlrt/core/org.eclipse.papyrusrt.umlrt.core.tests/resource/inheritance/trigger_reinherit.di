<?xml version="1.0" encoding="UTF-8"?>
<xmi:XMI xmi:version="2.0" xmlns:xmi="http://www.omg.org/XMI" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:architecture="http://www.eclipse.org/papyrus/infra/core/architecture" xmlns:di="http://www.eclipse.org/papyrus/0.7.0/sashdi">
  <di:SashWindowsMngr>
    <sashModel currentSelection="/0/@sashModel/@windows.0/@children.0">
      <windows>
        <children xsi:type="di:TabFolder">
          <children>
            <emfPageIdentifier href="papyrus.welcome:dynamic#/"/>
          </children>
          <children>
            <emfPageIdentifier href="trigger_reinherit.notation#_OTuQ4CRTEeeQgstmsPt_eQ"/>
          </children>
          <children>
            <emfPageIdentifier href="trigger_reinherit.notation#_Mx9twCRUEeeTJq9UWTPY2A"/>
          </children>
          <children>
            <emfPageIdentifier href="trigger_reinherit.notation#_N03VMCRUEeeTJq9UWTPY2A"/>
          </children>
        </children>
      </windows>
    </sashModel>
  </di:SashWindowsMngr>
  <architecture:ArchitectureDescription contextId="org.eclipse.papyrusrt.umlrt.architecture"/>
  <architecture:ArchitectureDescriptionPreferences>
    <viewpointIds>org.eclipse.papyrusrt.umlrt.viewpoint.basic</viewpointIds>
  </architecture:ArchitectureDescriptionPreferences>
</xmi:XMI>
