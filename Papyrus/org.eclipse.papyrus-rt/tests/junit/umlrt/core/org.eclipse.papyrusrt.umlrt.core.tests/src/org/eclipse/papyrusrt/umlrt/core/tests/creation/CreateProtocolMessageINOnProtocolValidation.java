...
/**
 * validation rules for creation of protocol message IN from protocol
 */
public class CreateProtocolMessageINOnProtocolValidation implements IValidationRule {

	public CreateProtocolMessageINOnProtocolValidation() {
	...
	public void validatePostEdition(Element targetContainer, Object[] commandResults) throws Exception {
		...
		Interface MessageSetIn = ProtocolUtils.getMessageSetIn((Collaboration) targetContainer);
		...
		Package protocolContainer = ProtocolUtils.getProtocolContainer((Collaboration) targetContainer);
	...
	protected Operation getOperation(Element targetContainer, Object[] commandResults) {
		if (!(targetContainer instanceof Collaboration)) {
			fail("targetcontainer is supposed to be a Collaboration");
		}
